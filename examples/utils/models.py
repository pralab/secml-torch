# model definition for kWTA defense
...
class Sparsify2D_vol(SparsifyBase):  # noqa: N801
    def __init__(self, sparse_ratio=0.5):
        super().__init__()
        self.sr = sparse_ratio
...
class SparseResNet(nn.Module):
    def __init__(self, block, num_blocks, sparsities, num_classes=10):
        ...
        self.layer1 = self._make_layer(
            block, 64, num_blocks[0], stride=1, sparsity=sparsities[0]
        )
        self.layer2 = self._make_layer(
            block, 128, num_blocks[1], stride=2, sparsity=sparsities[1]
        )
        self.layer3 = self._make_layer(
            block, 256, num_blocks[2], stride=2, sparsity=sparsities[2]
        )
        self.layer4 = self._make_layer(
            block, 512, num_blocks[3], stride=2, sparsity=sparsities[3]
        )
    ...
    def _make_layer(self, block, planes, num_blocks, stride, sparsity=0.5):
        ...
        layers = []
        for stride in strides:  # noqa: PLR1704
...
def SparseResNet18(sparsities=[0.5, 0.4, 0.3, 0.2]):  # noqa: N802, B006
