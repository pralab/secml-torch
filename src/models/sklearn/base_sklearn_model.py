...
class BaseSklearnModel(BaseModel):
	...
	def __init__(self, model: sklearn.base.BaseEstimator, preprocessing: Preprocessing = None):
	...
	def decision_function(self, x: torch.Tensor) -> torch.Tensor:
		...
		raise AttributeError("This model has neither decision_function nor predict_proba.")
	...
	def train(self, dataloader: DataLoader):
		if not isinstance(dataloader.dataset, SklearnDataset):
			raise ValueError(f'Internal dataset is not SklearnDataset, but {type(dataloader.dataset)}')
