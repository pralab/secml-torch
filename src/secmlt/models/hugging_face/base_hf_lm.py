...
class HFCausalLM(BaseLanguageModel):
    ...
    def __init__(
        ...
        tokenizer_kwargs : dict, optional
            Extra arguments for AutoTokenizer.from_pretrained().
        model_kwargs : dict, optional
            Extra arguments for AutoModelForCausalLM.from_pretrained().
        """
    ...
    def load(self) -> None:
        ...
        self._tokenizer = AutoTokenizer.from_pretrained(
            self._model_path, **self._tokenizer_kwargs
        )
        self._model = AutoModelForCausalLM.from_pretrained(
            self._model_path, **self._model_kwargs
        ).eval()
    ...
    def generate(self, prompts: list[list[dict]], **kwargs) -> list[str]:
        ...
        **kwargs
        ...
        enc = self._tokenizer(
            rendered_prompts, return_tensors="pt", padding=True, truncation=True
        )
    ...
    def logprobs(
        ...
        assert len(prompts) == len(targets), (
            "Prompts and targets must have the same length."
        )

        enc_p = self._tokenizer(
        ...
        input_ids = torch.cat([enc_p["input_ids"], enc_t["input_ids"]], dim=1).to(
            self._device
        )
        ...
        pad_id = self._tokenizer.pad_token_id
        tgt_lens = (
            (tgt_ids != pad_id).sum(dim=1)
            if pad_id is not None
            else tgt_ids.ne(-1).sum(dim=1)
        )
        prm_lens = enc_p["attention_mask"].sum(dim=1).to(self._device)

        batch_logps = []
        for b in range(input_ids.size(0)):
            p_len = int(prm_lens[b])
            t_len = int(tgt_lens[b])
            if t_len == 0:
                batch_logps.append(torch.empty(0, device=self._device))
                continue

            idx_pos = torch.arange(t_len, device=self._device) + (p_len - 1)
            idx_tok = tgt_ids[b, :t_len]
