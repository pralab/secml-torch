...
@pytest.fixture
def input_tensor():
    return torch.tensor([[0.2, 0.3], [0.0, -0.3]])


@pytest.fixture
def delta_tensor():
    return torch.tensor([[0.1, 0.2], [0.3, 0.2]])


@pytest.fixture
def domain_constraint():
    return MockConstraint(torch.tensor([[0.0, 0.0], [0.7, 0.8]]))


@pytest.fixture
def perturbation_constraint():
    return MockConstraint(torch.tensor([[0.1, 0.2], [0.3, 0.2]]))
...
def test_apply_domain_constraints(additive_manipulation, input_tensor):
    ...
    expected = torch.tensor([[0.0, 0.0], [0.7, 0.8]])
...
def test_apply_perturbation_constraints(additive_manipulation, delta_tensor):
    ...
    expected = torch.tensor([[0.1, 0.2], [0.3, 0.2]])
    assert torch.equal(result, expected), f"Expected {expected}, got {result}"
...
def test_additive_manipulation(additive_manipulation, input_tensor, delta_tensor):
    ...
    # expected delta is equal to expected_x_adv - input_tensor because of
    # the inversion of the manipulation
