...
from secmlt.adv.evasion.ddn import DDN, DDNNative
...
from src.secmlt.adv.evasion.advlib_attacks.advlib_ddn import DDNAdvLib
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "advlib",
            DDNAdvLib.get_perturbation_models(),
        ),
        (
            "native",
            DDNNative.get_perturbation_models(),
        ),
    ],
)
def test_ddn_attack(
    ...
    for perturbation_model in LpPerturbationModels.pert_models:
        if perturbation_model in perturbation_models_ddn:
            attack = DDN(
                perturbation_model=perturbation_model,
                num_steps=5,
                init_epsilon=1.0,
